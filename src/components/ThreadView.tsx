...
export const ThreadView: React.FC<ThreadViewProps> = ({ thread: initialThread, onBack }) => {
  ...
  const processSingleUpload = useCallback(async (uploadItem: any) => {
    ...
    // If payment is required (legacy/backward compatibility)
    if (response.status === 402) {
      throw new Error('Payment is no longer required but server still requested it. Please contact support.');
    }
          ...
          <>
            ...
            {thread.ipnsLink && (
              <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <h4 className="text-xs font-bold text-blue-600 uppercase mb-2">IPNS LINK</h4>
                <a
                  href={thread.ipnsLink.startsWith('http') ? thread.ipnsLink : `https://gateway.ipfs.io/ipns/${thread.ipnsLink}`}
                  target="_blank"
