...
import { POSTING_FEE } from '@/lib/constants';
...
export const usePostingPayment = () => {
  ...
  const processPayment = useCallback(async (amount?: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch('/api/payments/posting-fee', {
        ...
        body: JSON.stringify({
          ...
          amount: amount // Pass the requested amount
        }),
      });
      ...
      if (result.exempt) {
        setPaymentState({
          ...
          error: null,
          success: true,
          transactionSignature: 'exempt' // Special signature for exempt users
        });
        return 'exempt';
      }
      ...
      if (typeof window !== 'undefined' && 'solana' in window) {
        ...
        if (phantom?.isPhantom) {
          ...
          const verifyUrl = `/api/payments/posting-fee?signature=${signature}&fromWallet=${wallet.publicKey.toString()}${amount ? `&amount=${amount}` : ''}`;
