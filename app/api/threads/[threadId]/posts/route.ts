...
import { getThreadById, addCommentToThread } from '@/lib/storage';
...
export async function POST(
  ...
  try {
    ...
    const { content, image, video, ipfsCid, authorWallet, isAnonymous } = body;

    // Content, image, video, and ipfsCid are optional, but at least one must be provided
    if (!content && !image && !video && !ipfsCid) {
      return NextResponse.json(
        { error: 'Please provide either a comment, image, video, or IPFS CID' },
        { status: 400 }
      );
    }
