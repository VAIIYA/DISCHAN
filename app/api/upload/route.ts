...
// Replaced Storacha-based upload with blob store (in-memory) backed upload
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Store in in-memory blob store and return an id
    const blobId = BlobStore.save(filename, file.type, buffer);

    return NextResponse.json({
      url: `/api/files/${blobId}`,
      gatewayUrl: `https://ipfs.io/ipfs/${blobId}`,
      blobId: blobId,
      fileId: blobId,
      filename,
      fileType: file.type,
